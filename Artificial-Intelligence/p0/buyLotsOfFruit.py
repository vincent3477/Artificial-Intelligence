...
def buyLotsOfFruit(orderList):
    ...
    # *** Your Code Here ***
    total_sum = 0
    #check if value is none before adding to the total. If the value is none, abort the program
    for i in orderList:
        if i[0] not in FRUIT_PRICES:
            print(f"Fruit {i[0]} not recognized.")
            return None
        else:
            total_sum += FRUIT_PRICES.get(i[0]) * i[1]
    return total_sum
