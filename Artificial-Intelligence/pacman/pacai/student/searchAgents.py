...
class CornersProblem(SearchProblem):
    ...
    def isGoal(self, state):
        ...
        for g in self.corners:
            if g == (state[0]):
                new_corners_visited[i] = 1
            i += 1
        for goal_status in new_corners_visited:
            if goal_status == 0:
                return False
        return True
    ...
    def successorStates(self, state):
        ...
        for action in Directions.CARDINAL:
            ...
            if (not hitsWall):
                # Construct the successor.
                # Find the cost of the successor
                i = 0
                new_corners_visited = list(state[1])
                for g in self.corners:
                    if g == (x, y):
...
def cornersHeuristic(state, problem):
    ...
    approx_closest_corner = 999999
    for each_coor in corners:
        x_dist = (x - each_coor[0])
        if x_dist < 0:
            x_dist *= -1
        y_dist = (y - each_coor[1])
        if y_dist < 0:
            y_dist *= -1
        pyth_distance = x_dist + y_dist
        if approx_closest_corner < pyth_distance:
            approx_closest_corner = pyth_distance
        return approx_closest_corner
...
def foodHeuristic(state, problem):
    ...
    listFoods = foodGrid.asList()
    listFoods.sort()
    pyth_distance = 0
    distances = []
    if problem.heuristicInfo.get(state) is None:
        for each_food in listFoods:
            pyth_distance = maze(each_food, position, problem.startingGameState)
            distances.append(pyth_distance)
            problem.heuristicInfo[state] = max(distances, default=0)
    else:
        return problem.heuristicInfo.get(state)
